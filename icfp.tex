...
lazy evaluation but ignores linearity information. The second
semantics is instrumented with type information and is thus
\emph{linearity aware}, resulting in a stuck state when a linear
variable is used more than once. We show that for well-typed terms,
...
will result in a stuck state. The semantics is also type-aware (for
the purposes of proving type safety), defined by
