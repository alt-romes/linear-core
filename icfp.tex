...
\[
  \begin{array}{c}
    ...
    (\Theta' \mid e'[\pi/p]) \Downarrow (\Theta'' \mid v) : \sigma, \Sigma}
    {\Gamma ; \Delta \vdash (\Theta \mid e~\pi) \Downarrow (\Theta''
    \mid v) : \sigma , \Sigma  }\\[1.5em]
    \infer[]
    {\,}
    ...
    \\
    \Gamma, \ov{x_i :_\omega \sigma_i} , z:_{\Delta'} T ;\Delta , \ov{x_j :_1 \sigma_j} \vdash (\Theta \mid e) \Downarrow (\Theta' :
    K~\ov{x_i}) : T , e' : \tau ,\Sigma \\
    \Gamma ;\Delta \vdash (\Theta' \mid e'\ov{[x_i/y_i]}[K~\ov{x_i}/z]
    ) \Downarrow (\Theta'' \mid v) :\tau , \Sigma }
    {\Gamma ;\Delta \vdash (\Theta \mid
    \ccase{e}{z{:}_{\Delta'}T~\{{\dots,K~\ov{y_i} \to e', {\dots} }\}})
    ...
    {K : \ov{ \sigma_i
    \rightarrow_\omega \sigma_j \lolli} T\\
    \Gamma, z:_{\Delta'} T ;\Delta \vdash (\Theta \mid e) \Downarrow (\Theta' \mid
 ...
 \begin{definition}[Environment Expansion]
   ...
   \[
     \begin{array}{lcl}
       ...
       \left[ \Theta , x:_1\sigma = e' \mid e \right] & \triangleq  & (\lambda x_1:\sigma . [\Theta \mid e])~e'\\
       \left[\Theta , x:_\Delta \sigma = e' \mid e \right] & \triangleq & \llet{x:_\Delta \sigma = e'}{[\Theta \mid e]}
    \end{array}
   \]
 \end{definition}
...
\G ; \D \vdash [ \Theta \mid (e,\mathit{terms}(\Sigma))] : \tau \otimes \bigotimes(\Sigma)
 ...
 \begin{lemma}
   Let $\G ; \D , \ov{x_i :_{\Delta_i} \tau_i} ; \ov{x_j :_1 : \tau_j} \vdash  e : \sigma$, we have that
   $\G ; \D , \ov{\Delta_i} \vdash [ \ov{x_i :_{\Delta_i} \tau_i = e_i} , \ov{x_j :_1 : \tau_j = e_j} \mid e] : \sigma$
   \end{lemma}

\begin{theorem}[Type Preservation]
  Let $\G ; \D \vdash (\Theta \mid e) \Downarrow (\Theta' \mid v) : \tau , \Sigma$.
