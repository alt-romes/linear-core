...
to state and reason about a progress property.\todo{Na semantica O omega e 1 ainda estão separados, é assim?}
...
transformations.\todo{add part about operational semantics and proof being similar to them}
...
\G ; \D \vdash [ \Theta \mid (e,\mathit{terms}(\Sigma))] : \tau \otimes \bigotimes(\Sigma)
 ...
 \begin{lemma}~\label{lem:unwrapenv}
   ...
   \end{lemma}
\begin{proof}
By structural induction on the environment expansion definition.
\begin{description}
...
\begin{proof}
By structural induction on the instrumented operational semantics reductions.
\begin{description}
...
\begin{tabbing}
    ...
    (7) $\G; \D \vdash \Theta \mid y : \s, \Sigma$\`by reconstructing\\
...
\begin{tabbing}
    ...
    (9) $\G; \D \vdash \Theta \mid e : \tau,\Sigma$\` by reconstructing\\
    ...
    (13) $\G; \D \vdash \Theta' \mid \llet{x{:}_{\D_a}\s = v}{v} : \s, \Sigma$\`by reconstructing\\
...
\begin{tabbing}
    ...
    (14) $\G; \D \vdash \Theta \mid e'\ov{[x_{i,j}/y_{i,j}]}[K~\ov{x_{i,j}}/z] : \tau, \Sigma$\` by reconstructing\\
    ...
    (13) $\G; \D \vdash \Theta \mid e'\ov{[x_i/y_i]}[K~\ov{x_i}/z] : \tau, \Sigma$\` by reconstructing\\
    ...
    (10) $\G, z{:}_{\ov{y_j{:}_1\s_j}}\s, \ov{y_i{:}_\omega\s_i}; \D, \ov{y_j{:}_1\s_j} \vdash \Theta \mid e : \s, e':\tau, \Sigma$\`by reconstructing\\
    ...
    (18) $\G; \D \vdash \Theta' \mid e'\ov{[x_{i,j}/y_{i,j}]}[K~\ov{x_{i,j}}/z] : \tau, \Sigma$\`by reconstructing\\
    ...
    (9) $\G, z{:}_\cdot\s, \ov{y_i{:}_\omega\s_i}; \D \vdash \Theta \mid e : \sigma, e' : \tau, \Sigma$\` by reconstructing\\
    (10) $\G, z{:}_\cdot\s, \ov{y_i{:}_\omega\s_i}; \D \vdash \Theta \mid e : \sigma, e' : \tau, \Sigma$\` by reconstructing\\
    (11) $\G, z{:}_\cdot\s, \ov{y_i{:}_\omega\s_i}; \D \vdash \Theta' \mid K~\ov{x_i} : \sigma, e' : \tau, \Sigma$\` by induction hypothesis\\
    (12) $Let~\G'' = \G,\Theta'{\uparrow}^\omega~and~\D_d,\D_4,\D_5 = \D, \Theta'{\uparrow}^1$\` where $(\D_4, \D_5)$ types $(\Theta'{\downarrow^1_e}, \Sigma)$\\
    (12a) $\G''; \cdot \vdash K~\ov{x_i} : \s$\`since $K$ has no linear fields in this subcase\\
    (12b) $\G'', z{:}_{\cdot}\s, \ov{y_i{:}_\omega\s_i}; \D_d \vdash e' : \tau$\\
    (13) $\ov{\G''; \cdot \vdash x_{i} : \s_i}$\`by inv (12a)\\
    (14) $\G'', z{:}_{\cdot}\s; \D_d \vdash e'\ov{[x_i/y_i]} : \tau$\` by $\omega$-subst lemma\\
    (15) $\G''; \D_d \vdash e'\ov{[x_i/y_i]}[K~\ov{x_i}/z] : \tau$\` by $\D$-subst lemma\\
    (16) $\G; \D \vdash \Theta' \mid e'\ov{[x_i/y_i]}[K~\ov{x_i}/z] : \tau, \Sigma$\` by reconstructing\\
    (17) $\G; \D \vdash \Theta'' \mid v : \tau, \Sigma$\` by induction hypothesis\\
\end{tabbing}
...
\begin{tabbing}
    ...
    (9) $\G; \D, z{:}_1\s \vdash \Theta \mid e : \sigma, e' : \tau, \Sigma$\` by reconstructing\\
    ...
    (15) $\G;\D \vdash \Theta' \mid e'[K~\ov{x_i}/z] : \tau, \Sigma$\` by reconstructing\\
