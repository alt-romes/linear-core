...
guarantees linear resource usage via a type safety argument and prove
...
Both Haskell and its intermediate language Core are lazily
        ...
        As we make clear in this paper, \emph{lazy evaluation interacts non-trivially with linearity}.
...
the latter results from inlining $y$ in the let body of the former. Even though
the program no longer appears linear (as there are now two 
occurrences of $x$), the program is indeed linear
...
linear type system: it does not account for the lazy evaluation model
...
breaks down in the presence of laziness and designing a type system which,
...
contrast, a computation that depends on a linear resource, but is never run,
will not consume that resource.
Under the lens of non-strict evaluation, this means that a syntactic
...
non-strict and because the distinction between semantically and syntactically
...
programs that are semantically linear but are rejected by (the current) Core
linear type system.
% BETTER?:
...
semantically linear, but are not seen as linear by Core's (naive wrt lazy evaluation) linear type
...
\todo{Should we have a section starting with simple function application?}
...
%
Less trivially, let-bound expressions that capture linears variables
may be evaluated depending on the case-expression branch taken at runtime.
...
thunk, it will be evaluated only once when the binding is first forced and
