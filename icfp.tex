...
%
we turn our focus to linearity under \todo{either non strict simpler sentence or say focus on lazy. decide}non-strict evaluation, both since GHC Core is
...
semantics to be bisimilar to a Launchbury-style~\cite{10.1145/158511.158618} natural semantics.
...
a \rightarrow e\}}$ where $f = K$, but opaque.
\begin{enumerate}

    \item Consequently, cases with non-WHNF scrutinees must by typed such that no resource
    ...
    resources do end up being consumed (unless the result matches an
    unrestricted pattern), we must ensure any which transfer from the
    scrutinee to the alternative are ultimately consumed. (In the example,
    $x$ is not consumed by evaluating the scrutinee).

    Considering (1), we must resort to a more uniform method of guaranteeing we
    ``finish consuming`` the scrutinee. And there are only two ways of
...
resources, we can now present the final $Case_\textrm{WHNF}$ rule:
\[
\TypeCaseWHNF
\]
...
variable, we need now only to express that (1) the pattern-bound $\D$-variables can
...
if a $K$-pattern-bound $\D$-var is consumed, all remaining $K$-$\D$-vars,
standing for the remaining linear components of the same pattern, must also be
consumed.

To satisfy these two properties, we introduce a $\textit{Split}$ rule that
...
Thus, such a semantics is ill-suited for meta-reasoning about
linearity and so we develop an \emph{instrumented} semantics that is
enriched with sufficient information to allow us to establish type
safety, showing that linearity is preserved. We show that the natural
...
%
%Both semantics are
The natural semantics is equipped with an environment at assigns values to variables and is
mutated in order to express evaluation. Terms are explicit about
...
variable $x$ of the given type. The rules are standard, augmenting the
runtime environment with let-bound expressions which are evaluated
(only once) if the let-bound variable is forced.
