...
once} in mutual exclusion with the linear resources themselves
...
Mutually recursive let-bindings, i.e. bindings with definitions that
recursively depend on the bindings they define, behave similarly to
non-recursive lets as far as their usage in the let continuation is concerned.
...
%
% As far as we know, we are the first to prove optimisations preserve types in
% a non-strict linear language.
...
resources between the pattern-bound variables, %^as usage environments
...
scrutinee\footnote{In our calculus, the alternatives are assumed to be
  ...
  expression to progress evaluation.}. An expression in WHNF can either be %\begin{itemize}
% \item
...
\section{Instrumented Operational Semantics}
...
through let-binders.
Below we write $\llet{x{:}_{\Delta}\sigma = e,\dots,y{:}_{\Delta'}=e''
}{e'}$ for the iterated let-binding of variables $x , \dots , y$.       
\[
  \begin{array}{lclr}
    x^* & \triangleq & x\\
    ...
    (e~e')^* & \triangleq & \llet{x{:}_{\Delta}\sigma =
                            ...
                            $\Gamma;\Delta' \vdash e :
                            \sigma\rightarrow_\pi \varphi$ and
                                                   $\Gamma;\Delta \vdash e' : \sigma$}\\
    K~e_1~\dots~e_n & \triangleq & \llet{x_1:_{\Delta_1} \sigma_1 =
                                   (e_1)^*,\dots,x_n:_{\Delta_n}
                                   \sigma_n =
                                   (e_n)^*}{K~x_1~\dots~x_n} \\ &&
                                                               \qquad\mbox{with
                                                               $\Gamma;\Delta_1\vdash
...
\[
  \begin{array}{c}
    ...
    \infer[]
    {\,}
    {\Gamma : K~x_1~\dots~x_n \Downarrow \Gamma : K~x_1~\dots~x_n}
    \qquad
    \infer[]
    {\Gamma : e \Downarrow \Delta : K~x_1~\dots~x_n \quad
    \Delta : e'[x_i/y_i][K~x_1~\dots~x_n/z] \Downarrow \Theta : v }
    {\Gamma :
    \ccase{e}{z{:}_{\Delta'}\sigma~\{{\dots,K~\ov{y_i}, {\dots} \to e'}\}}
    \Downarrow \Theta : v}\\[1.5em]
    \infer[]
    {\Gamma : e \Downarrow \Delta : K~x_1~\dots~x_n \quad
    \Delta : e'[K~x_1~\dots~x_n/z] \Downarrow \Theta : v }
    {\Gamma :
...
\mid v) : \sigma$ where $\Gamma$ and $\Delta$ are typing judgments
...
its resulting value, both of type $\sigma$.
...
\[
  \begin{array}{c}
    ...
    \Lambda p.e) : \forall p . \sigma }
    \\[1.5em]
    \infer[]
    {\Gamma ; \Delta \vdash (\Theta \mid  e) \Downarrow (\Theta' \mid
    \Lambda p.e') : \forall p . \sigma \quad \Gamma ;\Delta \vdash
    (\Theta' \mid e'[\pi/p]) \Downarrow (\Theta'' : v) : \sigma}
    ...
    \rightarrow_\pi \tau }\\[1.5em]
    \infer[]
    {\Gamma;\Delta \vdash  (\Theta \mid e) \Downarrow (\Theta' \mid
    \lambda y:_\pi \sigma . e') : \sigma \rightarrow_\pi \tau
    \quad
    ...
    \mid v) : \tau}\\[1.5em]
       ...
       \infer[]
    {\Gamma , x:_{\Delta'} \sigma ; \Delta  \vdash (\Theta \mid e)
    \Downarrow (\Theta' \mid v) : \sigma}
    ...
    e}{e'})\Downarrow (\Theta' \mid v) : \tau
    }\\[1.5em]
    
    \infer[]
    {\,}
    {\Gamma ; \Delta \vdash (\Theta \mid K~x_1~\dots~x_n) \Downarrow
    (\Theta \mid K~x_1~\dots~x_n) : T}
    \\[1.5em]
    
    \infer[]
    {\Gamma;\Delta \vdash (\Theta \mid e) \Downarrow (\Theta' :
    K~x_1~\dots~x_n) : T \quad
    \Gamma ;\Delta \vdash (\Theta' \mid e'[x_i/y_i][K~x_1~\dots~x_n/z]
    ) \Downarrow (\Theta'' : v) :\tau }
    {\Gamma ;\Delta \vdash (\Theta \mid
    \ccase{e}{z{:}_{\Delta'}\sigma~\{{\dots,K~\ov{y_i}, {\dots} \to e'}\}})
    \Downarrow (\Theta'' \mid  v) : \tau}\\[1.5em]

    
    \infer[]
    {\Gamma ;\Delta \vdash (\Theta \mid e) \Downarrow (\Theta' \mid
    K~x_1~\dots~x_n) : T \quad
    \Gamma ;\Delta \vdash (\Theta' \mid  e'[K~x_1~\dots~x_n/z])
    \Downarrow (\Theta'' \mid v) : \tau }
    {\Gamma ; \Delta \vdash (\Theta \mid 
    \ccase{e}{z{:}_{\Delta'}\sigma~\{\dots , \_\to
    ...
    % : v_1 \dots  (\Gamma, \overline{x_i:_\omega \sigma_i = e_i}) : e_n
    % \Downarrow \Theta_n : v_n \quad
    (\Gamma ; \Delta \vdash (\Theta, \overline{x_i:_\omega \sigma_i =
