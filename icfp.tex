...
Finally, we consider default case alternatives, also known as wildcards
...
\todo[inline]{Previous explanation was very confusing, try to explain
  what the split is doing in terms of the example!}
...
\begin{lemma}[$\Delta$-bound to Linear]\label{lem:deltaone}
...
\renewcommand{\LinearDeltaRelationLemma}{
  \begin{lemma}[Linear to $\Delta$-bound]\label{lem:onedelta}
...
\begin{lemma}[Unrestricted and $\Delta$-bound]\label{lem:undelta}
...
\paragraph{$\Delta$-bound variables} Our development relies on the following characterization of $\D$-bound
variables:
...
usage environment $\D$, with $\D$ available in the linear context instead of
the linear variable (and the unrestricted context updated accordingly).
...
\begin{figure}[t]
  ...
  \[
   ...
   (\llet{\xD = e}{e'} )  \Longrightarrow \llet{\xD =       e}{e'[e/x]} &                    \mbox{Inlining}\\
      ...
      (\ccase{e}{\{\overline{\rho_j\to e_j}, \rho \to E[e_1], \overline{\rho_k\to e_k}\}} ) \qquad (\mbox{with $x$ fresh}) \\
     \Longrightarrow \llet{x = e_1}{\ccase{e}{\{\overline{\rho_j\to e_j}, \rho \to E[t], \overline{\rho_k\to e_k}\}}} 
    ...
    \end{array}
  \]}
  \caption{Optimising transformations validated by Linear Core\label{fig:opttrans} (see Appendix~\ref{app:optimisations} for details)} 
\end{figure}
