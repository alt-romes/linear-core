...
\begin{abstract}
Linear type systems can be used to guarantee (linear) resources are used exactly once.
...
of type systems by allowing them to enforce desirable usage patterns
for certain resources (e.g.~a
file handle). Typically, linear types can be used to enforce resources are used \emph{exactly once}.
%
...
Linearity in Haskell stands out from linearity in other
...
%
Core is a minimal, explicitly typed, lazy functional language with
both linear and unrestricted types, to which multiple
Core-to-Core optimising transformations are applied during
compilation. Notably Core does not have a (higher-order or otherwise)
store.
%
...
Despite the advantages of adding linear types to Core, the status quo is
...
evaluated under \emph{lazy evaluation} and $x$ is still consumed exactly once (when it is freed in the let body):
...
type system: it does not account for the non-strict evaluation model
...
A linear type system typically guarantees that linear resources are
consumed exactly once. Consequently, whether a program is
well-typed under this lens intrinsically depends on the
...
%
For instance, in the following Haskell-like program defining a linear
function \lstinline{f} (i.e., a function that should use its argument
exactly once), there are two syntactic
...
never evaluated (the non-linear \lstinline{x} goes unused).
%
\begin{lstlisting}
...
used in case alternatives, such as (we write $\lolli$ for linear
functions and \lstinline{->} for non-linear functions):
%
\begin{notyet}
\begin{lstlisting}
...
we must infer the superset of resources that may end up being consumed in a
...
%
It follows from the inferred resources used being the same for all bindings
...
More recently, \cite{DBLP:journals/pacmpl/ChoudhuryEEW21} and
\cite{cite:linearhaskell} both make use of usage-aware
semantics. Choudhury et al. do so in the context of graded dependent
...
call-by-need. \citet{cite:linearhaskell} provide a similar semantics
