...
system \ROUNDTWO{is designed to reconcile with the lazy} semantics of
...
\ROUNDTWO{Forcing a pattern variable evaluates the corresponding expression to
which the constructor was applied, and all linear variables captured by that
expression may }.
Otherwise, if we do not use the
pattern variables, then we do not run the computation and thus no resources are
consumed.
...
\subsubsection{Branching on WHNF-ness}\label{sec:onwhnf}
...
\subsubsection{Proof irrelevant resources}\label{sec:irrev}
