...
This document is a work in progress proposal that I will deliver (in
an extended form) at the end of
...
proposal on linting linearity in Core and to showcase the preliminary progress I
have made so far.
...
Since the publication of Linear Haskell~\cite{} and its release in GHC 9.0,
...
before being ultimately compiled to (untyped) machine code.
...
from trivial, and the implemented type system cannot accomodate
several optimising transformations that produce programs that violate
linearity in a syntactically (i.e.,~multiple occurrences of linear
...
linear linter, since disabling optimisations can incur significant
...
that that cannot sufficiently accommodate the
resulting programs.
...
information in order to to accommodate linear program in Core that
...
lets occur in Core due to optimising transformations that create let
...
We describe below an algorithm for computing the usage environment of a set of
...
valid programs and ultimately does not correctly capture the usage of a variable,
...
binder is used. However, we know that in the branch where z is used, z
is equal to False, and using False does not violate linearity since we can
