...
We propose to extend Core's formal type system with more linearity information
to be able to accommodate linear programs as seen in Core after optimising
transformations, prove its soundness/type safety, validate core-to-core
optimisation passes, and ultimately implement it into GHC's linter. Concretely,
we'll extend the linear Core system with rules based on usage annotations for
let, letrec and case binder bound variables, possibly a new kind of coercion for
multiplicities, and eventually other rules.
%
