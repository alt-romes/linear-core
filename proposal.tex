...
\section{Typing After Transformations}

\subsection{Result of desugar (before optimization)}

This resulting program fails lint with \emph{Linearity failure in lambda: x
'Many $\not\subseteq$ p}
\begin{code}
...
The case binder solution also applies here.
\textbf{case} $x$ evaluates x once, and then the binder has empty usage
environment because of the DEFAULT $x := []$. Then $f$ is applied to $x$ and $x$
means the usage of its usage environment which is empty: linearity is preserved

How are strictness annotations typed in the frontend? The issue is this program
...
coercions. Incorrect incompatible branch: CoAxBranch (src/Prelude/Linear/GenericUtil.hs:112:3-51): [S1
\begin{code}
...
\end{code}
In the first branch case, we pattern match on y to force it and then return $x$
rather than the constant $False$.
...
Solution with the (unifying) case binder idea:
Annotate the case binder with its usage environment. For enums without arguments
this usage environment will always be empty, meaning we can always use the case
binder instead of the actual enum without issues. This solves this problem as
...
Confusingly: the resulting optimised program from running the complete simplifier
and outputting with -ddump-simpl actually does what we expected it to regarding
the constants and linearity. So is the issue from running linear lint after a
