...
it on an invalid input. A common technique in Haskell is to use more expressive
types to assert properties about the values and get rid of the impossible cases
(e.g. a \textbf{NonEmpty} is a list that can not be empty). A well liked motto is
...
% TODO: But what are ADTs? Perhaps expand Haskell introduction section to
% include recursive ADTs
\textbf{GADTs}~\cite{10.1145/1159803.1159811,,,} are a very popular feature with
...
list \textbf{Cons~2~(Cons~1~Nil)} has type \textbf{Vec~(S~(S~Z))~Int} because
