...
families and (more recently) even impredicative polymorphism\cite{10.1145/3408971} have become
...
Note that if the list is empty, we terminate the program with an error. This
function is unsafe -- our program might crash if we use it on an invalid input.
A common technique in Haskell is to use more expressive types to assert
properties about the values and get rid of the unsafe cases (e.g. a
\textbf{NonEmpty} is a list that can not be empty). A well liked motto is "make
invalid states unrepresentable". In this light, we will introduce Generalized
Algebraic Data Types (GADTs) and create a list type indexed by size for which we
can write a completely safe \textbf{head} function. This is done by expressing that the size
of the list must be at least one at the type level.
...
constructors where the result type may differ in the type parameters across
branches. Pattern matching against GADTs can introduce local type refinements,
that is, pattern matching against certain data constructors will refine the type
information used for typechecking individual case alternatives. We develop
...
the \emph{DataKinds} language feature which promotes term-level constructors to
...
list \textbf{Cons~'a'~(Cons~'b'~Nil)} has type \textbf{Vec~(S~(S~Z))~Char}
because \textbf{Nil~::~Vec~Z~Char} and \textbf{Cons~'a'~Nil~::~Vec~(S~Z)~Char}.
GADTs make possible different data constructors being parametrized over
different type parameters as we do with \textbf{Vec}'s size parameter being
different depending on the constructor that constructs the list.

To define the safe \textbf{head} function, we must specify the type of the input
list taking into account that the size must not be zero. To that effect, the
function takes as input a \textbf{Vec (S n) a}, that is, a vector with size
...
\textbf{Nil} -- it would entail the argument list has size zero; however, it's
impossible to call \textbf{head} on a list of size zero!
