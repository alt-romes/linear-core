...
non-nullary\footnote{\textbf{IO} has kind $\textbf{Type}\to\textbf{Type}$,
...
constructor}} type constructor \textbf{IO}. Some of the example programs will look though as
...
Another defining feature of Haskell is its powerful type system. Features such
as type classes, arbitrary-rank types, functional dependencies, GADTs, type
families and (now) even impredicative polymorphism\cite{10.1145/3408971} have become
commonplace in Haskell code enforcing application level invariants and program
correctness through the types. For example, consider the definition of
\textbf{head} in the standard library, a function which takes the first element
of a list by pattern matching on the list constructors.
...
If the list is empty, we terminate the program with an error, otherwise, we take
the first element. This function is unsafe -- our program might crash if we use
it on an invalid input.
\todo[inline]{When applied to the empty list, ...}

A common technique in Haskell is to use more expressive
types to assert properties about the values and get rid of the impossible cases
(e.g. a \textbf{NonEmpty} is a list that can not be empty).

\todo[inline]{Leveraging Haskell's more advanced features, we can
  .... }

A well liked motto is
"make invalid states unrepresentable". In this light, we will introduce
Generalized Algebraic Data Types (GADTs) and create a list type indexed by size
for which we can write a completely safe \textbf{head} function by expressing
that the size of the list must be at least one, at the type level.


\textbf{GADTs}~\cite{10.1145/1159803.1159811,,,} are a very popular\todo{nope} feature with
which users can define data types as they would common recursive algebraic data
types, but that allows explicit type signatures for the data constructors where
the result type may differ in the type parameters across branches. Pattern
matching against GADTs can introduce local type refinements, that is, pattern
matching against certain data constructor will refine the type information known
for typechecking in individual case alternatives. We develop the length-indexed
lists example without discussing the type system and type inference details of
GADTs which we explore later in~\ref{related-work-gadts}.

% TODO: Explain Type in Type?

First, we define the natural numbers inductively: a natural number is either
zero (\textbf{Z}) or a successor (\textbf{S}) of another natural number (e.g.
the successor of the successor of zero is a natural number). The following
definition creates the \textbf{Nat} type, the \textbf{Z} and \textbf{S} data
constructors which construct values, \emph{and} the \textbf{Z} and \textbf{S}
\emph{type} constructors which construct types. The latter two exist because of
\emph{DataKinds} language feature which promotes term-level constructors to
...
is \textbf{Nat}. To construct a type of kind \textbf{Nat} we can only use the
type constructors \textbf{Z} and \textbf{S}. The second type parameter is the
...
existing list \emph{of size $m$} whose final size is $m+1$ (\textbf{S m}). The
list \texttt{Cons~2~(Cons~1~Nil)} has type \textbf{Vec~(S~(S~Z))~Int} because
\textbf{Nil~::~Vec~Z~Int} and \textbf{Cons~1~Nil~::~Vec~(S~Z)~Int}. GADTs make
possible different data type constructors being parametrized over different type
parameters as we do with \textbf{Vec}'s size parameter being different depending
on the constructor that constructs the list.
    To define the safe \textbf{head} function, we must specify the type of the
input list taking into account that the size must not be zero. To that effect,
the function takes as input a \textbf{Vec (S n) a}, that is, a vector with size
(n+1) for all possible n's, which makes it impossible to pass a list of type
\textbf{Vec Z a} to \textbf{head}.
...
Pattern matching on the \textbf{Nil} constructor would result in an innacessible
pattern warning because it is impossible to pass a list that would match
\textbf{Nil} -- it would entail the list has size zero; however, it's impossible
to call \textbf{head} on a list of size zero!

Linear types, which allow for a finer control over the usage of certain resources at the type
level, are another example of an extension to Haskell's type system, besides the aforementioned type-level
features, that makes it even more expressive.
...
\section{Linear Haskell\label{linear-haskell-section}}
