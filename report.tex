...
\textbf{GADTs}~\cite{10.1145/1159803.1159811,,,} are a wildly popular type level
feature that allow the user to define data types by enumerating its constructors
with explicit type signatures, where the result type may differ in the type
parameters across branches; pattern matching against GADTs might introduce type
refinements locally, i.e. in individual case alternatives. We develop the
length-indexed lists example without discussing the type system and type
inference details of GADTs which we explore later in~\ref{related-work-gadts}.
...
\emph{type} constructors which construct types. The latter two exist due to the
...
\begin{code}
    data Nat = Z | S Nat
\end{code}
% Before introducing the data type of 

Besides the aforementioned type-level features of Haskell, \emph{linear types},
which allow for a finer control over the usage of certain resources at the type
level, are another example of an extension to Haskell's type system that makes
it yet more expressive.
