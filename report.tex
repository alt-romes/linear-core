...
present a linearly typed lambda calculus\todo{citar o girard
  provavelmente ou o Barber (\url{https://www.lfcs.inf.ed.ac.uk/reports/96/ECS-LFCS-96-347/})}, a very simple language with linear
...
disjunction operator ($\oplus$)\todo{sum types} and the $\bang$
modality operator\todo{que serve para}. A typing
...
is denoted by an empty space\todo{juxtaposition} (\texttt{f~a} means \texttt{f} applied to
...
\texttt{head} in the standard library, a function which takes the first element
...
(e.g. a \texttt{NonEmpty} is a list that can not be empty).

\todo[inline]{Leveraging Haskell's more advanced features, we can .... }

A well liked motto is
"make invalid states unrepresentable". In this light, we will\todo{nao
  uses o futuro} introduce
Generalized Algebraic Data Types (GADTs) and create a list type indexed by size
for which we can write a completely safe \texttt{head} function by expressing
...
which users can define data types as they would common algebraic data
types, with the added ability to give explicit type signatures for the data
constructors where the result type may differ in the type parameters across
branches.
\todo{nao ha branches, é nos varios construtores. This allows for
  additional type invariants to be represented with GADTs through
  their type parameters, which restricts the use of specific
  constructors and their subsequent deconstruction through pattern
  matching}
...
\todo{acho que a outra frase que escrevi e melhor que esta / complementa}
We develop
