...
% BNF grammars
\usepackage[center]{formal-grammar}
...
\begin{itemize}
    ...
    % TODO: \item values in rust are linear by default while non-linear is the
        % haskell default?
\end{itemize}

Core was imbued with linear types because an implementation detail required it,
and, more importantly, to guarantee linear programs remain linear after being
...
However, currently, typechecking linearity in Core is broken in the face of
optimizations! The reason is not evidently clear: if we can typecheck linearity
in the surface level Haskell why can't we in Core? In fact, if we were to
typecheck the programs in Core as they were at the surface level, we would
succeed, but programs are \emph{transformed} (by semantic-preserving optimizing
transformations) and hence end up different from how they were originally
written. For example, the following recursive let definition, where $x$ is a
linear variable that must be used exactly once, would not typecheck in a source
Haskell program since the typechecker cannot tell that $x$ is used linearly, but
this program might occur naturally in Core from transformations on a program
that did succesfully typecheck:
\begin{code}
letrec f = \case
        True -> f False
        False -> x
...
\begin{grammar}
    \firstcase{\t}{\star}{Unit type}
    \otherform{t \tensor t'}{Multiplicative conjunction type}
    % t ::= a $\lolli$ b
\end{grammar}
