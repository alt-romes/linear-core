...
examples of anonymous functions, try to understand how they could be disregarded
as incorrect, and work our way up to linear types from them. The first receives a
file handle as an argument, ignores it, and returns $\star$ (read unit), which
...
And, similarly, the second receives a file handle and closes it
twice. Note that \textbf{close} has type $\textbf{Handle} \to \star$, that is, it's a
function that receives a handle and returns unit.
\[
    \lambda h.~\textrm{close}~h;~\textrm{close}~h;
\]

Both the above functions share the same type ($\textbf{Handle} \to \star$),
under a simple type system such as C's they are type correct, and both have
erroneous behaviours. The first forgets to close the handle and the second
closes it twice. Our goal is now to reach a type system that rejects these
two programs.

The key observation is that the argument of the function must be linear, and we
can augment the common function type to account for the linearity of its
argument. We take the function type $A \to B$ and replace the arrow with the
$\lolli$
%
\footnote{Since linear types are born from a
correspondence\cite{curry-howard} (the Curry-Howard isomrphism) with linear
logic\cite{linear-logic}, we borrow the $\lolli$ symbol and other linear logic
connectives to describe linear types.}
%
...
% Linear types are a powerful idea
