...
defining that linear resources must be used exactly once (linearly). In
programming languages with a linear type system, not using certain resources or
using them twice are type errors. Linear types can, thus, be used, e.g., to
guarantee file handles, socket descriptors and allocated memory are freed
exactly once (leaks and double-frees become type errors)\cite{}, implement safe
high-performance language interop\cite{}, guarantee quantum entangled states are
not duplicated~\cite{}, handle mutable state\cite{}, a bit on session
types guarantees\cite{}, and much more.

% TODO: Also include bit about how it makes the basis of other type systems such
% as rusts owneship. How ?

As an example, consider the following program which allocates some memory and
frees it twice. The variable $p$ is a linear resource created by the
\textbf{malloc} keyword.
...
using it in both calls to \textbf{free}. The example program doesn't typecheck!
In section~\ref{linear-types} we describe the fundamentals of linear types in
more detail with a formal definition and many examples.

Despite their usefulness, most mainstream programming languages don't have
linear types. In practice, it can be hard to balance linear and non-linear types
and avoid the boilerplate threading of linear resources that we'll see ahead.
