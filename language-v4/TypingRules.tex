...
\newcommand{\TypeCaseWHNF}{
    \mprset{flushleft}
    \infer[(CaseWHNF)]
    {\textrm{e is in \emph{WHNF}} \\ \judg[\G][\ov{\D_i}]{e}{\s}
    \\ \ov{\G,\var[z][\ov{\D_i}];\ov{\D_i},\D' \vdash_{alt} \rho\to e' \s \Longrightarrow \vp \prescript{\dag}{}{:^z_{\ov{\D_i}}}}}
...
\newcommand{\TypeCaseNotWHNF}{
    ...
    {\textrm{e is definitely not in \emph{WHNF}} \\
    \judg{e}{\s}
    \\ \ov{\judg[\G,z{:}_{\irr{\D}}\s][\irr{\D},\D']{\rho\to e'}{\s \Longrightarrow \vp}[alt][\irr{\D}][z][\ddag]}}
    {\splitjudg{\ccase{e}{\z[\irr{\D}]~\{\ov{\rho \to e'}\}}}{\vp}}
    % TODO: Add that [d1] entails moving all delta usages to [delta] usages in all vars i d1
}
...
\newcommand{\TypeAltNNotWHNF}{
    ...
    {\judg{K~\ov{\xo},\ov{y_i{:}_1\s_i}^n\to e}{\s \Longrightarrow \vp}[alt][\D_s][z][\ddag]}
}
...
\newcommand{\TypeAltNWHNF}{
    ...
    {\judg{K~\ov{\xo},\ov{y_i{:}_1\s_i}^n\to e}{\s \Longrightarrow \vp}[alt][\ov{\D_i}^n][z][\dag]}
}
