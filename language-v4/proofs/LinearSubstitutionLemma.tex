...
\begin{tabbing}
  ...
  Subcase $x$ doesn't occur in $e$ nor in $e''$\\
  TODO:
\end{tabbing}
...
\begin{tabbing}
  ...
  Subcase $x$ occurs in $e$\\
    ...
    (5) $\ov{\judg[\subst{\G}{\irr{\D,\xl}}{\D,\xl},\zr{\irr{\D,\xl}}{\s'}][\irr{\D,\xl},\D'']{\rho\to e''}{\s' \Longrightarrow \vp}[alt][\irr{\D,\xl}]}$\`by inv.\\
    (6) $\judg[\subst{\G}{\D'}{x}][\D,\D']{e[e'/x]}{\vp}$\`by i.h.\\
    (7) $\ov{\judg[\subst{(\subst{\G}{\D'}{x})}{\irr{\D,\D'}}{\D,\D'},\zr{\irr{\D,\D'}}{\s'}][\irr{\D,\D'},\D'']{\rho\to e''[e'/x]}{\s' \Longrightarrow \vp}[alt][\D,\D']}$\\
  ...
  Subcase $x$ occurs in $\ov{e''}$\\
    ...
    (5) $\ov{\judg[\subst{\G}{\irr{\D}}{\D},\zr{\irr{\D}}{\s'}][\irr{\D},\D'',\xl]{\rho\to e''}{\s' \Longrightarrow \vp}[alt][\irr{\D}]}$\`by inv.\\
    (6) $e[e'/x] = e$\` by $x$ does not occur in $e$\\
    (7) $\ov{\judg[\subst{(\subst{\G}{\D'}{x})}{\irr{\D}}{\D},\zr{\irr{\D}}{\s'}][\irr{\D},\D'',\D']{\rho\to e''[e'/x]}{\s' \Longrightarrow \vp}[alt][\irr{\D}]}$\`by i.h.\\
