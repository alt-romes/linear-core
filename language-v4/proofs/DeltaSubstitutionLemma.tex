...
\begin{tabbing}
  (1) $\G; \D \vdash e' : \s$\\
  (2) $\G,\xD; \D,\D' \vdash_{alt} \konstructor~\to e :^z_{\D_s} \s' \Rightarrow \vp$\\
  (3) $n > 0$ \` by inv.\\
  (4) $\ov{\D_i} = \ov{\lctag{\D_s}{K_j}}$\`by inv.\\
  (5) $\G,\xD,\ov{\xo},\ov{y_i{:}_{\D_i}\s_i}; \D, \D' \vdash e : \vp$\` by inv.\\
  (6) $\G,\ov{\xo},\ov{y_i{:}_{\D_i}\s_i}; \D, \D' \vdash e[e'/x] : \vp$\` by i.h.\\
  (7) $\G; \D,\D' \vdash_{alt} \konstructor~\to e[e'/x] :^z_{\D_s} \s' \Rightarrow \vp$\` by $AltN$\\
\end{tabbing}
