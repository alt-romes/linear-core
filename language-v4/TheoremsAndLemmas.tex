...
\begin{theorem}[Beta-reduction on multiplicity abstractions preserves types]~\\
    If $\G \vdash (\Lambda p.~e)~\pi : \vp$ then $\G \vdash e[\pi/p] : \vp$
\end{theorem}
}
...
\begin{theorem}[Inlining preserves types (and thus is sound)]~\\
If $\G \vdash \llet{\xD = e}{e'} : \vp$ then $\G \vdash \llet{\xD = e}{e'[e'/x]} : \vp$
\end{theorem}
}
